...
Toma `alpha` como el mínimo valor y `beta` el máximo valor en x; estos están como 0 y 2, respectivamente.

```{r}
plot_uniform_distribution(alpha=2, beta = 4)
```
...
Toma `min` como valor mínimo en x y `max` como valor máximo; estos están como 0 y 1, respectivamente.

```{r}
plot_triangular_distribution(mean = 2, sd = 2)
```
